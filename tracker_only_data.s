...
; Special version of tracker.s
; for mod2abc --only-data
...
tracker_init:
	ld a,(song_speed)
	; force new row at start
	xor a
	ld (tracker_row_countdown),a

	; force new pattern at start
	xor a
...
; a must be (tracker_pattern_countdown)
tracker_do_row:
	cp (hl)
	jp nz,.no_matching_row
...
tracker_pattern_list:
	defw 0
; pointer to row within current pattern
